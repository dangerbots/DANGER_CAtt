...
async def _(event):
    ...
    survivor = await edit_or_reply(event, "BSDK tera gf h na ek ...!")
    animation_chars = [
    ...
    for i in animation_ttl:
        ...
        await survivor.edit(animation_chars[i % 21])


@bot.on(admin_cmd(outgoing=True, pattern=f"gey$"))
...
async def _(event):
    ...
    survivor = await event.edit(event, "👁👁")
    animation_chars = [
        "👁👁\n  👄  =====> Abey NOBITA Gay",
        "👁👁\n  👅  =====> Abey VISHAL Gay",
        "👁👁\n  💋  =====> Abey GOKU Gay",
    ...
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
        await survivor.edit(animation_chars[i % 11])
